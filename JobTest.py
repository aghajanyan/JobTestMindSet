...
with open ("newimages/floors.txt") as f:
...
train_in1 = np.array(train_in);
train_out1 =   np.array(train_out); 

train_in1 = np.mean(train_in1, axis=3);
print(train_in1.shape);

"""-------------"""

train_in1 = train_in1.reshape(train_in1.shape[0], 256, 256, 1);
...
t = time.time();
model.fit(train_in1, train_out1, batch_size=10, epochs=350);
t = time.time() - t;
